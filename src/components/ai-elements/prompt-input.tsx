...
export const PromptInputSubmit = ({
  ...
  return (
    <Button
      className={cn('gap-1.5 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90', className)}
      size={size}
      type="submit"
      variant={variant}
      {...props}
...
export type PredefinedPromptsProps = {
  prompts: string[]
  onSelectPrompt: (prompt: string) => void
}

export const PredefinedPrompts = ({ prompts, onSelectPrompt }: PredefinedPromptsProps) => {
  // === Display Loading State While Prompts Are Being Generated ===
  // Shows placeholder buttons while dynamic prompts are being fetched from backend
  const isLoading = prompts.length === 0

  return (
    <div className="flex flex-wrap gap-2 p-3">
      {isLoading ? (
        // === Loading Skeleton: Display placeholder buttons while fetching prompts ===
        <>
          <div className="px-3 py-2 text-sm rounded-lg bg-muted/50 border border-input/50 animate-pulse min-w-32 h-9" />
          <div className="px-3 py-2 text-sm rounded-lg bg-muted/50 border border-input/50 animate-pulse min-w-40 h-9" />
        </>
      ) : (
        // === Render Dynamic Prompts: Display fetched prompts as clickable buttons ===
        prompts.map((prompt, index) => (
          <button
            ...
            className={cn(
              ...
              'bg-background border-input hover:bg-accent hover:border-accent text-foreground',
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
            )}
            type="button"
            title={prompt} // Show full text on hover
          >
